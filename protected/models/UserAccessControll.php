...
class UserAccessControll
{
    ...
    public function getModule($moduleKey, $field='')
    {
        ...
        $moduleData = $moduleObj->find('ModuleCode = ? AND IsPublish = 1', array($moduleKey));
    ...
    {
        $obj = new UserRole();
        $data = $obj->find('RoleType = ? AND ParentId=0 AND IsPublish = 1', array($Module['module_code']));
        ...
        foreach ($collection['controllers'] as $controller) {
            foreach($controller['actions'] as $action) {
                if(isset($action['actions'])) {
                    ...
                    $model=UserRules::model()->find("ModuleId='".$collection['module']['id']."'
                        AND RoleId='".$this->getRoleByModule($collection['module'], 'id')."' 
                        AND PrivilegesController='".$controller['name']."'
                        AND PrivilegesActions='".$actionStr."'
                    ...
                    $model->ModuleId = $collection['module']['id'];
                    $model->RoleId = $this->getRoleByModule($collection['module'], 'id');
                    $model->PrivilegesController = $controller['name'];
                    $model->PrivilegesActions = $actionStr;
                    $model->PermissionType = $userStr;
