...
class NewsSubCategoryController extends Controller {
    ...
    public function actionCreate() {
        $model = new NewsSubCategory;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['NewsSubCategory'])) {
            $model->attributes = $_POST['NewsSubCategory'];
            $model->InsertedDate = User::model()->getCurrentDateTime();
            if ($model->save())
                $this->redirect(array('view', 'id' => $model->NewsSubCategoryId));
        }
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('NewsSubCategory');
    ...
    public function actionAdmin() {
        $model = new NewsSubCategory('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['NewsSubCategory']))
            $model->attributes = $_GET['NewsSubCategory'];
     ...
     * @return NewsSubCategory the loaded model
     * @throws CHttpException
     */
    public function loadModel($id) {
        $model = NewsSubCategory::model()->findByPk($id);
        if ($model === null)
     ...
     * @param NewsSubCategory $model the model to be validated
     */
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'news-sub-category-form') {
     ...
     */
    public function actionDeleteMultipleSubCategories() {
        //p($_POST);
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            $anDeletedIds = isset($_POST['Data']) ? $_POST['Data'] : array();
            //p($anDeletedIds);
            if (count($anDeletedIds) > 0) {
                $bDeleted = NewsSubCategory::removeSelectedSubCategories($anDeletedIds);
                if ($bDeleted) {
                    echo '1';
                    exit;
                }
     ...
     */
    public function actionChangeSubCategoriesStatus() {
        //p($_POST);
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            ...
            if (count($anStatusIds) > 0) {
                $bUpdated = NewsSubCategory::changeStatusSelectedSubCategories($anStatusIds, $anStatusType);
                if ($bUpdated) {
                    echo '1';
                    exit;
                }
